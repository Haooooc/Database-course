...
class TestSearch:
    ...
    def pre_run_initialization(self):
        ...
        if self.book_info.get('content') is not None:
            self.content_in_store = self.book_info['content'].split("\n")[0]
        else:
            self.content_in_store = ""
        yield
    ...
    def test_search_global_not_exist(self):
        ...
        assert self.auth.search_book(title=not_exist) == 522
        assert self.auth.search_book(content=not_exist) == 522
        assert self.auth.search_book(tag=not_exist) == 522    
    ...
    def test_search_in_store_not_exist(self):
        not_exist = "Do not exist ^o^"
        assert self.auth.search_book(title=not_exist, store_id=self.store_id) == 522
        assert self.auth.search_book(content=not_exist, store_id=self.store_id) == 522
        assert self.auth.search_book(tag=not_exist, store_id=self.store_id) == 522
